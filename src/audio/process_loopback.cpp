...
#else
  // Fallback for Windows SDK < 10.0.20348 (header was introduced in Win11 SDK).
  // 进程级 loopback 在运行时通过 ActivateAudioInterfaceAsync 拿到接口,
  // 只要 OS 是 Win11 21H2+ 就能用 —— 即便编译时 SDK 不带这个头。
